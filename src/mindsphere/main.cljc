(ns mindsphere.main
  "Kotodama WASM entrypoint for the Mindsphere clean-room actor (L4) — Clojure port.
  ...
  facts (`mindsphere.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mindsphere")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mindsphere.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mindsphere-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mindsphere-compat/ready)
